...
func chHostPort(host string) string {
...
func doJSONEachRowPOST(ctx context.Context, endpoint string, payload any, user string, pass string) error {
...
func InsertEvent(ctx context.Context, cfg Config, event model.Event) error {
	clickhouse := chHostPort(cfg.ClickHouseHost)
	...
	if err := doJSONEachRowPOST(ctx, endpoint, payload, cfg.ClickHouseUser, cfg.ClickHousePass); err != nil {
...
func InsertCDCUser(ctx context.Context, cfg Config, row model.CDCUserRow) error {
	clickhouse := chHostPort(cfg.ClickHouseHost)
	db := cfg.ClickHouseDB
	if db == "" {
		db = "appdb"
